...
LayerLichSuPhienTXMini::LayerLichSuPhienTXMini()
{
...
bool LayerLichSuPhienTXMini::init()
{
    if (!Layer::init())
    {
        return false;
    ...
    auto rootNode = CSLoader::getInstance()->createNode("LayerLichSuPhienTXMini.csb");
...
//    }
    ...
    if(btnClose){
        btnClose->addClickEventListener(CC_CALLBACK_1(LayerLichSuPhienTXMini::onBtnClose, this));
    }
    
    this->btnPre = static_cast<Button*>(pnlBg->getChildByName("btnPre"));
    if(this->btnPre){
        this->btnPre->addClickEventListener(CC_CALLBACK_1(::LayerLichSuPhienTXMini::onBtnPrev, this));
        this->btnPre->setPressedActionEnabled(true);
    }
    
    this->btnNext = static_cast<Button*>(pnlBg->getChildByName("btnNext"));
    if(this->btnNext){
        this->btnNext->addClickEventListener(CC_CALLBACK_1(LayerLichSuPhienTXMini::onBtnNext, this));
        this->btnNext->setPressedActionEnabled(true);
    }
...
//    this->txtName->setString("");
    ...
    ImageView * imgTai = static_cast<ImageView*>(pnlBg->getChildByName("imgTai")) ;
    imgTai->loadTexture(StringUtils::format("design/LayerTaiXiuMini/%stai-1.png",SceneManager::getSingleton().getFolderLang().c_str()));
    ImageView * imgXiu = static_cast<ImageView*>(pnlBg->getChildByName("imgXiu")) ;
    imgXiu->loadTexture(StringUtils::format("design/LayerTaiXiuMini/%sxiu-1.png",SceneManager::getSingleton().getFolderLang().c_str()));
    ...
    boost::shared_ptr<IRequest> request(new ExtensionRequest(EXT_EVENT_TX_GAME_HIS_REQ, params));
...
void LayerLichSuPhienTXMini::reloadCoin(int type){
//    if (type == 0){
...
void LayerLichSuPhienTXMini::tableCellTouched(TableView* table, TableViewCell* cell)
{
    CCLOG("cell touched at index: %ld", cell->getIdx());
}

Size LayerLichSuPhienTXMini::tableCellSizeForIndex(TableView *table, ssize_t idx)
{
    return Size(590, 70);
}

TableViewCell* LayerLichSuPhienTXMini::tableCellAtIndex(TableView *table, ssize_t idx)
{
    ...
    if (table->getTag() == 1){
        ...
        if (!cell) {
            ...
            LayerItemLichSuPhienTXMiniTai* his = LayerItemLichSuPhienTXMiniTai::create();
        ...
        {
            LayerItemLichSuPhienTXMiniTai* his = (LayerItemLichSuPhienTXMiniTai*)cell->getChildByTag(123);
            if (his){
                his->setDatas(it.stt,it.time,it.name,it.tai,it.xiu,it.hoan);
            }
        }
        ...
        if (!cell) {
            ...
            LayerItemLichSuPhienTXMiniXiu* his = LayerItemLichSuPhienTXMiniXiu::create();
        ...
        {
            LayerItemLichSuPhienTXMiniXiu* his = (LayerItemLichSuPhienTXMiniXiu*)cell->getChildByTag(123);
            if (his){
                his->setDatas(it.stt,it.time,it.name,it.tai,it.xiu,it.hoan);
            }
        }
...
ssize_t LayerLichSuPhienTXMini::numberOfCellsInTableView(TableView *table)
{
    if (table->getTag() == 1)
...
void LayerLichSuPhienTXMini::onBtnClose(Ref* pSender){
    this->removeFromParentAndCleanup(true);
}
void LayerLichSuPhienTXMini::OnExtensionResponse(unsigned long long ptrContext, boost::shared_ptr<BaseEvent> ptrEvent){
    ...
    if (strcmp(EXT_EVENT_TX_GAME_HIS_DETAIL_RESP, cmd->c_str()) == 0){
...
//        }
        ...
        this->txtTongTai->setString(mUtils::convertMoneyEx(atof(lstHis[7].c_str())));
        this->txtTongXiu->setString(mUtils::convertMoneyEx(atof(lstHis[8].c_str())));
    ...
    else if(strcmp(EXT_EVENT_TX_GAME_HIS_RESP, cmd->c_str()) == 0){
        boost::shared_ptr<string> strhis = param->GetUtfString("ghs");
        if (strhis != NULL){
...
string LayerLichSuPhienTXMini::getTime(string str){
...
string LayerLichSuPhienTXMini::GetUrlXiNgau(int number) {
    string num = "";
    switch (number){
        case 1:
...
void LayerLichSuPhienTXMini::loadHis(){
    for (int i = 0; i< this->lstMatch.size();i++ ){
        if (strcmp(this->phienid.c_str(),this->lstMatch[i].mid.c_str())==0){
            ...
            boost::shared_ptr<IRequest> request(new ExtensionRequest(EXT_EVENT_TX_GAME_HIS_DETAIL_REQ, params));
            GameServer::getSingleton().Send(request);
        }
...
string LayerLichSuPhienTXMini::getDay(string str){
    time_t tt = atoi(str.c_str());
    struct tm * ptm = localtime(&tt);
    char buf[30];
...
void LayerLichSuPhienTXMini::setCurrentItem(int tag){
...
void LayerLichSuPhienTXMini::setLstHistory(vector<string> list){
...
bool LayerLichSuPhienTXMini::onTouchBegan(cocos2d::Touch *pTouch, cocos2d::Event *pEvent)
{
    return true;
}
void LayerLichSuPhienTXMini::onBtnPrev(Ref* pSender){
    this->lstHistoryMatchTai.clear();
...
void LayerLichSuPhienTXMini::onBtnNext(Ref* pSender){
    this->lstHistoryMatchTai.clear();
    this->tblListHisTai->reloadData();
    this->lstHistoryMatchXiu.clear();
    this->tblListHisXiu->reloadData();
...
void LayerLichSuPhienTXMini::onEnter()
{
    Layer::onEnter();
    auto dispatcher = Director::getInstance()->getEventDispatcher();
    auto listener = EventListenerTouchOneByOne::create();
    listener->setSwallowTouches(true);
    listener->onTouchBegan = CC_CALLBACK_2(LayerLichSuPhienTXMini::onTouchBegan, this);
...
void LayerLichSuPhienTXMini::onExit()
{
    Layer::onExit();
    auto dispatcher = Director::getInstance()->getEventDispatcher();
...
bool LayerItemLichSuPhienTXMiniTai::init()
{
    if (!Layer::init())
    {
        return false;
    ...
    auto root = CSLoader::getInstance()->createNode("LayerItemLichSuPhienTai.csb");
...
void LayerItemLichSuPhienTXMiniTai::setDatas(int stt,string time,string name,string tai,string xiu,string hoan){
...
bool LayerItemLichSuPhienTXMiniXiu::init()
{
    if (!Layer::init())
    {
        return false;
    ...
    auto root = CSLoader::getInstance()->createNode("LayerItemLichSuPhienXiu.csb");
    ui::Helper::doLayout(root);
    this->addChild(root);
...
void LayerItemLichSuPhienTXMiniXiu::setDatas(int stt,string time,string name,string tai,string xiu,string hoan){
