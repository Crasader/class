#include "AvatarInGameCatte.h"
#include "SceneManager.h"
#include "GameServer.h"
#include "mUtils.h"
#include "ChanUtils.h"
#include "PhomMessDef.h"
LayerAvatarInGameCatte::LayerAvatarInGameCatte()
{

}

LayerAvatarInGameCatte::~LayerAvatarInGameCatte(){
	log("Destructor LayerAvatarInGameCatte");
}

bool LayerAvatarInGameCatte::init() {
	if (!Layer::init())
		return false;
	isGuess = false;
...
void LayerAvatarInGameCatte::setIcon(const int& pos, char *url){
	if (this->getUserByPos(pos) == NULL)
		return;
	this->getUserByPos(pos)->setIcon(url);
}

void LayerAvatarInGameCatte::setFlag(const int& pos, bool isShow){
	if (this->getUserByPos(pos) == NULL)
		return;
	this->getUserByPos(pos)->setFlag(isShow);
}

void LayerAvatarInGameCatte::setName(const int& pos, string name){
	if (this->getUserByPos(pos) == NULL)
		return;
	this->getUserByPos(pos)->setName(name);
}

void LayerAvatarInGameCatte::setMoney(const int& pos, string money){
	if (this->getUserByPos(pos) == NULL)
		return;
	this->getUserByPos(pos)->setMoney(money);
}

void LayerAvatarInGameCatte::setMoney(const int& pos, int money) {
	if (this->getUserByPos(pos) == NULL)
		return;
	this->getUserByPos(pos)->setMoney(money);
}

void LayerAvatarInGameCatte::setMoney(const int& pos, double money) {
	if (this->getUserByPos(pos) == NULL)
		return;
	this->getUserByPos(pos)->setMoney(money);
}

void LayerAvatarInGameCatte::setReady(const int& pos, bool isReady){
	if (this->getUserByPos(pos) == NULL)
		return;
	this->getUserByPos(pos)->setReady(isReady);
}

void LayerAvatarInGameCatte::setVisibleLayerInvite(const int& pos, bool isShow) {
	Avatar52La *user = this->getUserByPos(pos);
	if (user != NULL)
		user->setVisibleLayerInvite(isShow);
}

void LayerAvatarInGameCatte::setUnReadyAllUser() {
...
void LayerAvatarInGameCatte::resetGame() {
	this->setUserCount(0);
	setUnReadyAllUser();
}

void LayerAvatarInGameCatte::setListUserByPhom(string listUser) {
	//resetGame();
	this->typeGame = PHOM;
...
void LayerAvatarInGameCatte::setListUserByTienLen(string listUser) {
...
void LayerAvatarInGameCatte::formatAndStore(const char &c1, const char &c2) {
	char1 = c1;
	char2 = c2;
...
Avatar52La* LayerAvatarInGameCatte::getUserByPos(const int& pos){
	if (pos < 0)
		return NULL;
...
int LayerAvatarInGameCatte::getIndexInArrByName(string& name) const {
...
int LayerAvatarInGameCatte::getIndexInArrByAccountID(string aI) {
	for (int i = 0; i < (int)arrAI.size(); i++) {
		if (strcmp(arrAI[i].c_str(), aI.c_str()) == 0) return (i < 5 ? i : -1); // trả lại giá trị -1 nếu tên nằm ngoài danh sách 4 người đầu tiên
	}
	return -1;
}

int LayerAvatarInGameCatte::getPosByName(string pName) {
...
int LayerAvatarInGameCatte::getPosByAccountID(string aI) {
...
string LayerAvatarInGameCatte::getNameByPos(int pPos) {
	int pos = getIndexInArrByName(this->myName);
...
string LayerAvatarInGameCatte::getAccountIDByPos(int pPos) {
	int pos = getIndexInArrByAccountID(this->myAI);
...
void LayerAvatarInGameCatte::updateUsers() {
...
void LayerAvatarInGameCatte::runTimer(const int& posUser) {
	stopAllTimer();
...
void LayerAvatarInGameCatte::stopAllTimer() {
...
bool LayerAvatarInGameCatte::isSpectator() {
...
void LayerAvatarInGameCatte::resetAll(){
...
bool LayerAvatarInGameCatte::isStartedGame()
{
...
void LayerAvatarInGameCatte::stopAllBlinkAvatar(){
...
void LayerAvatarInGameCatte::runTimerWithTime(const int& posUser, const float& time)
{
	stopAllTimer();
	Avatar52La *avatar = getUserByPos(posUser);
	if (avatar == NULL) return;
...
void LayerAvatarInGameCatte::showAvatarMe(int tag, string userName)
{
...
void LayerAvatarInGameCatte::removeUserFromList(string _name)
{
...
void LayerAvatarInGameCatte::setResult(int pos, int rescode, int type, bool isOnGame, double money)
{
	Avatar52La *user = getUserByPos(pos);
	if (user == NULL)
		return;
...
void LayerAvatarInGameCatte::setListUserGuess(string _list, char charSplit)
{
	...
	int length = arrUser.size() > (5 - this->_count) ? 5 - this->_count : arrUser.size();
	if (length > 0){
		this->resetGuessUser();
	}
	for (int i = 0; i < length; i++)
...
Avatar52La* LayerAvatarInGameCatte::getEmptyAvatar()
{
	for (int i = 10; i >= 0; i--){
...
void LayerAvatarInGameCatte::resetGuessUser()
{
	for (int i = 0; i < 10; i++)
	{
		Avatar52La *user = this->getUserByPos(i);
...
void LayerAvatarInGameCatte::checkSameIP()
{
