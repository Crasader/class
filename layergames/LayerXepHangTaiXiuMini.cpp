...
LayerXepHangTaiXiuMini::LayerXepHangTaiXiuMini()
{
...
bool LayerXepHangTaiXiuMini::init()
{
    if (!Layer::init())
    {
        return false;
    ...
    auto rootNode = CSLoader::getInstance()->createNode("LayerXepHangTaiXiuMini.csb");
...
//    }
    ...
    Image_20->loadTexture(StringUtils::format("design/LayerTaiXiuMini/%shead-bxh.png",SceneManager::getSingleton().getFolderLang().c_str()));
    Button* btnClose = dynamic_cast<Button*>(pnlBg->getChildByName("btnClose"));
    if(btnClose){
        btnClose->addClickEventListener(CC_CALLBACK_1(LayerXepHangTaiXiuMini::onBtnClose, this));
    }
    ...
    boost::shared_ptr<IRequest> request(new ExtensionRequest(EXT_EVENT_TX_RANK_REQ, params));
...
void LayerXepHangTaiXiuMini::tableCellTouched(TableView* table, TableViewCell* cell)
{
    CCLOG("cell touched at index: %ld", cell->getIdx());
}

Size LayerXepHangTaiXiuMini::tableCellSizeForIndex(TableView *table, ssize_t idx)
{
    return Size(1239, 75);
}

TableViewCell* LayerXepHangTaiXiuMini::tableCellAtIndex(TableView *table, ssize_t idx)
{
    ...
    if (!cell) {
        ...
        LayerItemRankTXMini* his = LayerItemRankTXMini::create();
    ...
    {
        LayerItemRankTXMini* his = (LayerItemRankTXMini*)cell->getChildByTag(123);
        if (his){
            his->setDatas(it.stt,it.name,it.money);
        }
    }
...
ssize_t LayerXepHangTaiXiuMini::numberOfCellsInTableView(TableView *table)
{
    
    return lstRank.size();
}
void LayerXepHangTaiXiuMini::onBtnClose(Ref* pSender){
    this->removeFromParentAndCleanup(true);
}
void LayerXepHangTaiXiuMini::OnExtensionResponse(unsigned long long ptrContext, boost::shared_ptr<BaseEvent> ptrEvent){
    ...
    if (strcmp(EXT_EVENT_TX_RANK_RESP, cmd->c_str()) == 0){
...
string LayerXepHangTaiXiuMini::getTime(string str){
...
string LayerXepHangTaiXiuMini::getResult(string str){
...
bool LayerXepHangTaiXiuMini::onTouchBegan(cocos2d::Touch *pTouch, cocos2d::Event *pEvent)
{
    return true;
}

void LayerXepHangTaiXiuMini::onEnter()
{
    Layer::onEnter();
    auto dispatcher = Director::getInstance()->getEventDispatcher();
    auto listener = EventListenerTouchOneByOne::create();
    listener->setSwallowTouches(true);
    listener->onTouchBegan = CC_CALLBACK_2(LayerXepHangTaiXiuMini::onTouchBegan, this);
...
void LayerXepHangTaiXiuMini::onExit()
{
    Layer::onExit();
    auto dispatcher = Director::getInstance()->getEventDispatcher();
...
bool LayerItemRankTXMini::init()
{
    ...
    auto root = CSLoader::getInstance()->createNode("LayerItemXepHangTXMini.csb");
...
void LayerItemRankTXMini::setDatas(int stt,string name,string money){
    if (SceneManager::getSingleton().getGameVersion() == GameVersion::p2) {
