...
bool PopStar::init()
{
    ...
    for (int row = 0; row < ROW_NUM; ++row)
    {
        for (int col = 0; col < COL_NUM; ++col)
        {
            ...
            {
//                star->setScale(2);
                //star->setContentSize(Size(STAR_WIDTH, STAR_HEIGHT));
                star->setPos(Vec2(START_POS + STAR_WIDTH*col + STAR_WIDTH/2, STAR_HEIGHT*row + STAR_HEIGHT/2 + row*20+col*5));
                star->setDestPos(Vec2(START_POS + STAR_WIDTH*col + STAR_WIDTH/2, STAR_HEIGHT*row + STAR_HEIGHT/2));
...
void PopStar::onClick(const cocos2d::Vec2& pos)
{
    if (gameLayer == NULL) return;
    int row = pos.y / STAR_HEIGHT;
    int col = (pos.x - START_POS) / STAR_WIDTH ;
    if (row >= ROW_NUM || col >= COL_NUM || col < 0 || row < 0)
    {
        return;
    }
...
void PopStar::gotoNextLevel()
{
    ...
    getPopStarDataMgr().setLevel(level);
    getPopStarDataMgr().setHistoryLevelScore(level, getPopStarDataMgr().getScore());
