...
PopStarDataMgr::PopStarDataMgr()
{
    ...
    historyScore = UserDefault::getInstance()->getIntegerForKey(HISTORY_TOTAL_SCORE.c_str(), 0);
    
    int historyMaxLevel = UserDefault::getInstance()->getIntegerForKey(HISTORY_MAX_LEVEL.c_str(), 1);
    for (int i = 0; i < historyMaxLevel; ++i)
    {
        char szBuf[64] = {0};
        snprintf(szBuf, 64, HISTORY_LEVEL_SCORE.c_str(), i+1);
        int score = UserDefault::getInstance()->getIntegerForKey(szBuf, 0);
...
int PopStarDataMgr::getHistoryLevel()
{
    return UserDefault::getInstance()->getIntegerForKey(HISTORY_MAX_LEVEL.c_str(), 1);
}
...
void PopStarDataMgr::setHistoryScore(int score)
{
    ...
    {
        historyScore = score;
        UserDefault::getInstance()->setIntegerForKey(HISTORY_TOTAL_SCORE.c_str(), score);
        UserDefault::getInstance()->setIntegerForKey(HISTORY_MAX_LEVEL.c_str(), gameLevel);
...
void PopStarDataMgr::setHistoryLevelScore(int level, int score)
{
    ...
    if (level <= (int)historyLevelScores.size())
    {
        ...
        UserDefault::getInstance()->setIntegerForKey(szBuf, score);
        UserDefault::getInstance()->flush();
    }
...
void PopStarDataMgr::setCurrScore(int core){
    currScore = core;
}
int PopStarDataMgr::setCurrScore(){
    return currScore;
}
