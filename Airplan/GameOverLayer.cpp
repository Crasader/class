...
bool GameOverLayer::init()
{
	...
	do 
	{
	    ...
	    Sprite* background = Sprite::create("shoot_background3.png");
		background->setPosition(Point(winSize.width/2, winSize.height/2));
		this->addChild(background);

        Label* lblGameOver = Label::createWithSystemFont("GameOver", "", 30);
        lblGameOver->setColor(Color3B(143,146,147));
        lblGameOver->setPosition(Point(winSize.width/2, winSize.height/2 + 200));
        this->addChild(lblGameOver);
        
        
		auto normalBackToGame = Sprite::create("Play again.png");
		auto pressedBackToGame = Sprite::create("Play again.png");
        ...
        lblGameOver->runAction(sequence);
        
        
        Label* lblHighScore = Label::createWithSystemFont("Highest Score:", "", 40);
        lblHighScore->setColor(Color3B(143,146,147));
        lblHighScore->setPosition(Point(20,30));
        lblHighScore->setAnchorPoint(Point::ANCHOR_MIDDLE_LEFT);
		...
		highestScore->setPosition(Point(290,30));
