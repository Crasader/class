...
#if defined(_1UNICODE) || defined(_1UNICODE)
 ...
 * </ul> */
typedef struct XMLDLLENTRY XMLNodeEx
{
  private:
    ...
    XMLNodeEx(struct XMLNodeDataTag *pParent, XMLSTR lpszName, char isDeclaration);
    /// Constructors are protected, so use instead one of: XMLNode::parseString, XMLNode::parseFile, XMLNode::openFileHelper, XMLNode::createXMLTopNode
    XMLNodeEx(struct XMLNodeDataTag *p);
  ...
  public:
    ...
    static XMLNodeEx parseString   (XMLCSTR  lpXMLString, XMLCSTR tag=NULL, XMLResults *pResults=NULL);
    ...
    static XMLNodeEx parseFile     (XMLCSTR     filename, XMLCSTR tag=NULL, XMLResults *pResults=NULL);
    ...
    static XMLNodeEx openFileHelper(XMLCSTR     filename, XMLCSTR tag=NULL);
    ...
    XMLNodeEx getParentNode() const;                                 ///< return the parent node
    XMLNodeEx getChildNode(int i=0) const;                           ///< return ith child node
    XMLNodeEx getChildNode(XMLCSTR name, int i)  const;              ///< return ith child node with specific name (return an empty node if failing). If i==-1, this returns the last XMLNode with the given name.
    XMLNodeEx getChildNode(XMLCSTR name, int *i=NULL) const;         ///< return next child node with specific name (return an empty node if failing)
    XMLNodeEx getChildNodeWithAttribute(XMLCSTR tagName,
                                      XMLCSTR attributeName,
                                      XMLCSTR attributeValue=NULL,
                                      int *i=NULL)  const;         ///< return child node with specific name/attribute (return an empty node if failing)
    XMLNodeEx getChildNodeByPath(XMLCSTR path, char createNodeIfMissing=0, XMLCHAR sep='/');
                                                                   ///< return the first child node with specific path
    XMLNodeEx getChildNodeByPathNonConst(XMLSTR  path, char createNodeIfMissing=0, XMLCHAR sep='/');
                                                                   ///< return the first child node with specific path.
    ...
    XMLNodeEx deepCopy() const;                                      ///< deep copy (duplicate/clone) a XMLNode
    static XMLNodeEx emptyNode();                                    ///< return XMLNode::emptyXMLNode;
    /** @} */

    ~XMLNodeEx();
    XMLNodeEx(const XMLNodeEx &A);                                     ///< to allow shallow/fast copy:
    XMLNodeEx& operator=( const XMLNodeEx& A );                        ///< to allow shallow/fast copy:

    XMLNodeEx(): d(NULL){};
    static XMLNodeEx emptyXMLNode;
     ...
     * @ingroup xmlModify
     * @{ */
    static XMLNodeEx createXMLTopNode(XMLCSTR lpszName, char isDeclaration=FALSE);                    ///< Create the top node of an XMLNode structure
    XMLNodeEx        addChild(XMLCSTR lpszName, char isDeclaration=FALSE, XMLElementPosition pos=-1); ///< Add a new child node
    XMLNodeEx        addChild(XMLNodeEx nodeToAdd, XMLElementPosition pos=-1);                          ///< If the "nodeToAdd" has some parents, it will be detached from it's parents before being attached to the current XMLNode
     ...
     * @{ */
    static XMLNodeEx createXMLTopNode_WOSD(XMLSTR lpszName, char isDeclaration=FALSE);                     ///< Create the top node of an XMLNode structure
    XMLNodeEx        addChild_WOSD(XMLSTR lpszName, char isDeclaration=FALSE, XMLElementPosition pos=-1);  ///< Add a new child node
    ...
    XMLElementPosition positionOfChildNode(XMLNodeEx x) const;
    ...
    static char setGlobalOptions(XMLCharEncoding characterEncoding=XMLNodeEx::char_encoding_UTF8, char guessWideCharChars=1,
  ...
  private:
      ...
      typedef struct XMLNodeDataTag // to allow shallow copy and "intelligent/smart" pointers (automatic delete):
      {
          ...
          XMLNodeEx                *pChild;         // Array of child nodes
      ...
      XMLNodeEx addChild_priv(int,XMLSTR,char,int);
